...
public:
    ...
    void deleteSubscriber(string);
    void deletePublisher(string);
...
void Database::deleteFromTable(string sql)
{
    char *messageError;
...
string Database::getSubscribers(string publisherId)
{
    string getid("select subscriber_id from pub_sub where publisher_id='" + publisherId  + "';");
    Records records = getContents(getid);
    string subscriberId = "";
    for (auto &record : records)
    {
        // do something with your records
        subscriberId = subscriberId + record[0] + "\n";
    }
    return subscriberId;
}

void Database::deleteSubscriber(string subscriberId)
{
    string sql("delete from subscriber_details where subscriber_id=2;'" + subscriberId  + "';");
    deleteFromTable(sql);
}
void Database::deletePublisher(string publisherId)
{
    string sql("delete from publisher_details where subscriber_id=2;'" + publisherId  + "';");
    deleteFromTable(sql);
}
/*int main(){
